...
said = " ".join(sys.argv[1:]) or "الشحنة ما وصلت، والسائق ما رد علي"
...
if not agent_id:
    ...
    audio = voho("/v1/speech", {"text": "أهلاً بك في النخبة للنقل، معك ليلى. كيف أقدر أساعدك؟", "voice": "layla", "model": "sada-1", "format": "mp3"}, raw=True)
...
out = voho("/v1/agents/%s/reply" % agent_id, {
    "text": said,
    "variables": {"company": os.environ.get("VOHO_COMPANY", "النخبة للنقل")},
})
